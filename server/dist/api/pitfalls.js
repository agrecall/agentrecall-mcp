...
router.post('/', authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        // 5. 插入或更新数据库
        const result = await upsertPitfall(sanitizedPattern, sanitizedWorkaround, embeddingVector, taxonomy, contextFingerprint, errorSignature, user.instanceId);
        // 6. 记录提交日志
        await pool.query(`INSERT INTO submissions (instance_id, pitfall_id, action, ip_address, request_payload, processing_time_ms)
...
router.get('/stats', async (_req, res) => {
    ...
    try {
       ...
       ORDER BY date DESC`);
        // 获取热门分类
        const categoryResult = await pool.query(`SELECT 
