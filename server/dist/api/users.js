...
router.post('/register', async (req, res) => {
    try {
        // 检查是否开放注册
        const configResult = await pool.query("SELECT config_value FROM system_configs WHERE config_key = 'enable_registration'");
        ...
        // 获取默认 API 配额
        const quotaResult = await pool.query("SELECT config_value FROM system_configs WHERE config_key = 'default_api_quota'");
        const defaultQuota = quotaResult.rows.length > 0 ? parseInt(quotaResult.rows[0].config_value) : 1000;
        // 加密密码
        const passwordHash = await bcrypt.hash(password, 10);
        // 创建用户
        const userResult = await pool.query(`INSERT INTO users (email, password_hash, username, api_quota) 
       VALUES ($1, $2, $3, $4) RETURNING id, email, username, role, created_at`, [email, passwordHash, username, defaultQuota]);
...
router.post('/login', async (req, res) => {
    try {
        ...
        // 查询用户
        const userResult = await pool.query('SELECT id, email, username, password_hash, role, status, api_quota, api_used FROM users WHERE email = $1', [email]);
...
router.get('/me', authenticateUser, async (req, res) => {
    try {
        const user = req.user;
        const userResult = await pool.query(`SELECT id, email, username, role, status, api_quota, api_used, last_login_at, created_at 
       FROM users WHERE id = $1`, [user.userId]);
        ...
        // 获取 API Key 数量
        const apiKeyCount = await pool.query('SELECT COUNT(*) FROM api_keys WHERE user_id = $1 AND is_active = TRUE', [user.userId]);
        res.json({
            success: true,
            user: {
                id: userData.id,
...
router.get('/', authenticateUser, requireAdmin, async (req, res) => {
    try {
        ...
        params.push(limit);
        params.push(offset);
        const usersResult = await pool.query(`SELECT id, email, username, role, status, api_quota, api_used, last_login_at, created_at 
...
router.put('/:id/status', authenticateUser, requireAdmin, async (req, res) => {
    try {
        ...
        // 记录管理员操作
        await pool.query(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, ip_address) 
       VALUES ($1, $2, $3, $4, $5, $6)`, [currentUser.userId, 'update_user_status', 'user', id, JSON.stringify({ newStatus: status }), req.ip]);
...
router.put('/:id/quota', authenticateUser, requireAdmin, async (req, res) => {
    try {
        ...
        const currentUser = req.user;
        await pool.query(`INSERT INTO admin_logs (admin_id, action, target_type, target_id, details, ip_address) 
       VALUES ($1, $2, $3, $4, $5, $6)`, [currentUser.userId, 'update_user_quota', 'user', id, JSON.stringify({ newQuota: quota }), req.ip]);
