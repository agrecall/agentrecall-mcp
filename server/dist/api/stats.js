...
router.get('/system', authenticateUser, requireAdmin, async (req, res) => {
    try {
        ...
        // 获取最近 7 天的 API 调用趋势
        const trendResult = await pool.query(`SELECT DATE(created_at) as date, COUNT(*) as request_count
       FROM api_usage_logs
       WHERE created_at >= CURRENT_DATE - INTERVAL '7 days'
       GROUP BY DATE(created_at)
       ORDER BY date DESC`);
        // 获取热门端点
        const endpointsResult = await pool.query(`SELECT endpoint, COUNT(*) as request_count
       FROM api_usage_logs
       WHERE created_at >= CURRENT_DATE - INTERVAL '7 days'
       GROUP BY endpoint
...
router.get('/user', authenticateUser, async (req, res) => {
    try {
       ...
       ORDER BY date DESC`, [user.userId]);
        ...
        // 获取用户 Top Endpoints
        const topEndpointsResult = await pool.query(`SELECT endpoint, COUNT(*) as request_count
       FROM api_usage_logs
       WHERE user_id = $1
       GROUP BY endpoint
       ORDER BY request_count DESC
...
router.get('/usage', authenticateUser, async (req, res) => {
    try {
        ...
        params.push(limit);
        params.push(offset);
        const logsResult = await pool.query(`SELECT l.id, l.endpoint, l.method, l.status_code, l.duration_ms, l.created_at,
              u.email as user_email, ak.key_name as api_key_name
...
router.get('/history', authenticateUser, async (req, res) => {
    try {
        ...
        params.push(limit);
        params.push(offset);
        const historyResult = await pool.query(`SELECT ch.id, ch.session_id, ch.request_type, ch.status,
...
router.get('/dashboard', authenticateUser, async (req, res) => {
    try {
        ...
        try {
            // 检查时区是否存在
            const tzCheck = await pool.query(`SELECT EXISTS (SELECT 1 FROM pg_timezone_names WHERE name = $1)`, [userTimezone]);
        ...
        const todayStatsResult = await pool.query(`SELECT COUNT(*) as request_count
       FROM api_usage_logs
       WHERE user_id = $1 
         AND created_at >= (DATE_TRUNC('day', NOW() AT TIME ZONE $2) AT TIME ZONE $2)`, [user.userId, validTimezone]);
        // 获取本月统计（正确计算用户时区的本月开始时间）
        const monthStatsResult = await pool.query(`SELECT COUNT(*) as request_count
       FROM api_usage_logs
       WHERE user_id = $1 
         AND created_at >= (DATE_TRUNC('month', NOW() AT TIME ZONE $2) AT TIME ZONE $2)`, [user.userId, validTimezone]);
        // 获取 API Keys 列表
        const apiKeysResult = await pool.query(`SELECT id, key_name, key_prefix, permissions, usage_count, is_active, last_used_at, created_at
       FROM api_keys
       WHERE user_id = $1
