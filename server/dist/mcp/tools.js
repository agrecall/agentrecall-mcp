 ...
 * 提交避坑指南到知识网络
 */
async function handleSubmitPitfall(input) {
    ...
    try {
        ...
        // 4. 插入或更新数据库
        const result = await upsertPitfall(sanitizedPattern, sanitizedWorkaround, embedding, input.taxonomy, input.contextFingerprint, input.errorSignature, input.instanceId);
        // 5. 记录提交日志
        await pool.query(`INSERT INTO submissions (instance_id, pitfall_id, action, request_payload, processing_time_ms)
 ...
 * 查询相似的避坑指南
 */
async function handleQueryPitfall(input) {
    const startTime = Date.now();
    try {
        let results = [];
        // 1. 优先使用向量搜索（如果提供了 embedding）
        if (input.embedding) {
            const vectorResults = await searchSimilarPitfalls(input.embedding, input.similarityThreshold, input.limit);
        ...
        if (results.length === 0 && input.errorSignature) {
         ...
         LIMIT $2`, [input.errorSignature, input.limit]);
        ...
        if (results.length === 0 && input.contextFingerprint) {
         ...
         LIMIT $2`, [input.contextFingerprint, input.limit]);
 ...
 * 验证服务器健康状态
 */
async function handleVerifyHealth(_input) {
    const startTime = Date.now();
    try {
        // 检查数据库连接
