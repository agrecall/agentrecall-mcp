...
function isHighEntropy(str, threshold = 4.5) {
    ...
    if (str.length < 16 || str.length > 128)
        return false;
    // 检测是否包含足够多的不同字符
 ...
 * @returns 脱敏后的字符串
 */
function structureSanitize(input) {
    try {
        // 尝试解析为 JSON
 ...
 * @returns 脱敏后的字符串
 */
function entropySanitize(input) {
    // 分割字符串（保留分隔符）
 ...
 * @returns 脱敏后的对象
 */
export function sanitizeObjectInput(obj) {
 ...
 * @returns 脱敏后的数组
 */
export function sanitizeArrayInput(arr) {
    return arr.map(item => {
