 ...
 * @returns 限流结果
 */
export async function checkRateLimit(identifier, limitType, maxRequests, windowSeconds) {
    try {
 ...
 * @returns 限流状态
 */
export async function getRateLimitStatus(identifier, limitType) {
    try {
        const client = await getRedisClient();
        const config = RATE_LIMIT_CONFIGS[limitType];
 ...
 * @param limitType - 限流类型
 */
export async function resetRateLimit(identifier, limitType) {
    try {
        const client = await getRedisClient();
        const key = `ratelimit:${limitType}:${identifier}`;
 ...
 */
async function logRateLimit(identifier, limitType, currentCount, limit, isBlocked) {
    try {
        const { pool } = await import('../db/index.js');
        await pool.query(`INSERT INTO rate_limit_logs (identifier, limit_type, current_count, limit_count, window_start, is_blocked)
       VALUES ($1, $2, $3, $4, NOW(), $5)`, [identifier, limitType, currentCount, limit, isBlocked]);
    }
    catch (error) {
        console.error(JSON.stringify({
            level: 'error',
 ...
 * @returns Express 中间件
 */
export function createRateLimitMiddleware(limitType, getIdentifier) {
 ...
 * @returns Express 中间件
 */
export function ipRateLimit(limitType) {
 ...
 * @returns Express 中间件
 */
export function instanceRateLimit(limitType) {
    return createRateLimitMiddleware(limitType, (req) => {
        const user = req.user;
