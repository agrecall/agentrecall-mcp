 ...
 * 发送 SSE 事件
 */
function sendSSEEvent(clientId: string, event: string, data: any): void {
 ...
 * 广播 SSE 事件到所有客户端
 */
function broadcastSSEEvent(event: string, data: any): void {
 ...
 * 处理 Initialize 请求
 */
async function handleInitialize(params: any, clientId?: string): Promise<JSONRPCResponse> {
...
async function handleToolsCall(params: any): Promise<JSONRPCResponse> {
  ...
  if (!handler) {
    return createErrorResponse(
      ...
      `Tool handler not implemented: ${name}`
    );
  }
 ...
 * 处理 JSON-RPC 请求
 */
async function handleJSONRPCRequest(request: any, clientId?: string): Promise<JSONRPCResponse> {
  // 验证 JSON-RPC 格式
