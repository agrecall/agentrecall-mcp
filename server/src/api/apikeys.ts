...
export async function verifyApiKey(key: string): Promise<any | null> {
  ...
  const result = await pool.query(
    `SELECT ak.*, u.id as user_id, u.email, u.status as user_status, u.api_quota, u.api_used
     FROM api_keys ak
...
export async function logApiKeyUsage(
  ...
  // 更新用户使用次数
  await pool.query(
    'UPDATE users SET api_used = api_used + 1 WHERE id = $1',
    [userId]
  );
  
  // 记录使用日志
  await pool.query(
    `INSERT INTO api_usage_logs (user_id, api_key_id, endpoint, method, status_code, duration_ms) 
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [userId, apiKeyId, endpoint, method, statusCode, durationMs]
  );
}
...
router.get('/', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    const apiKeysResult = await pool.query(
      `SELECT id, key_name, key_prefix, permissions, rate_limit, usage_count, 
...
router.post('/', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    // 检查用户 API Key 数量限制
    const maxKeysResult = await pool.query(
      "SELECT config_value FROM system_configs WHERE config_key = 'max_api_keys_per_user'"
    );
    const maxKeys = maxKeysResult.rows.length > 0 ? parseInt(maxKeysResult.rows[0].config_value) : 5;
    
    const currentKeysResult = await pool.query(
    ...
    const apiKeyResult = await pool.query(
      `INSERT INTO api_keys (user_id, key_name, key_hash, key_prefix, permissions, rate_limit, expires_at) 
       VALUES ($1, $2, $3, $4, $5, $6, $7) 
       RETURNING id, key_name, key_prefix, permissions, rate_limit, expires_at, created_at`,
      [user.userId, name, hash, prefix, JSON.stringify(permissions), rateLimit, expiresAt]
    );
 ...
 * 获取 API Key 使用统计
 */
router.get('/:id/usage', authenticateUser, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
    const { id } = req.params;
...
router.get('/admin/all', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    // 获取 API Key 列表
    const apiKeysResult = await pool.query(
      `SELECT ak.*, u.email as user_email, u.username as user_username
       FROM api_keys ak
       JOIN users u ON ak.user_id = u.id
