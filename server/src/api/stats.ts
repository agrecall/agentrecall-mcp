...
router.get('/usage', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) FROM api_usage_logs l ${whereClause}`,
      params
    );
    ...
    const logsResult = await pool.query(
      `SELECT l.id, l.endpoint, l.method, l.status_code, l.duration_ms, l.created_at,
...
router.get('/history', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    const historyResult = await pool.query(
      `SELECT ch.id, ch.session_id, ch.request_type, ch.status,
